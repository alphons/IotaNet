...
namespace IotaNet.Iri.Service
{
	public class TipsManager
	{
		...
		private  Logger log = LoggerFactory.getLogger<TipsManager>();
    ...
    private int RATING_THRESHOLD = 75; // Must be in [0..100] range
		...
		public TipsManager( Tangle tangle,
	...
	Hash entryPoint(final Hash reference, final Hash extraTip, final int depth)
	{
	...
	Hash markovChainMonteCarlo(final Hash tip, final Hash extraTip, final Map<Hash, Long> ratings, final int iterations, final int maxDepth, final Set<Hash> maxDepthOk, final Random seed) 
	{
...
void serialUpdateRatings(final Hash txHash, final Map<Hash, Long> ratings, final Set<Hash> analyzedTips, final Hash extraTip)
{
...
ratings.put(currentHash, rating);
    ...
    Set<Hash> updateHashRatings(Hash txHash, Map<Hash, Set<Hash>> ratings, Set<Hash> analyzedTips) 
{
	Set<Hash> rating;
    ...
    long recursiveUpdateRatings(Hash txHash, Map<Hash, Long> ratings, Set<Hash> analyzedTips) 
	{
		long rating = 1;
		if (analyzedTips.add(txHash))
		{
	...
	boolean belowMaxDepth(Hash tip, int depth, Set<Hash> maxDepthOk) 
	{
